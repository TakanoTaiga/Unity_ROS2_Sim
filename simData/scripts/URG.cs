...
public class URG : MonoBehaviour
{
    ...
    void FixedUpdate() {
        ...
        for(int i = 0; i < laser_volume ; i++){
            float angle_rad = angle_increment * (float)i + angle_min - this.transform.eulerAngles.y * (Mathf.PI / 180);
        ...
        if(ros2unity.Ok()){
            if(ros2node == null){
                ...
                pub = ros2node.CreatePublisher<sensor_msgs.msg.LaserScan>("urg/laser");
            }
